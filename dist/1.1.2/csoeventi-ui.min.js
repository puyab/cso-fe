/*! Csoeventi ui Widget v1.1.2 | MIT License | powerby https://github.com/sajadweb */
class CsoeventiUi{holidays=[];static init(e){let t=new CsoeventiUi(e);return(async()=>{t.render()})(),t}constructor(e){this.config=e,this.isOpen=!1,this.user=null,this.id=e?.id||"csoeventi-ui",window.apiUrl=this.apiUrl=e?.url||"https://api-cso.zagrosagency.xyz",this.date=new Date,this.year=this.date.getFullYear(),this.month=this.date.getMonth(),this.months=["January","February","March","April","May","June","July","August","September","October","November","December"]}log(...e){this.config?.debug&&console.log("CsoeventiUi",e)}renderSuccess(){document.getElementById(this.id).innerHTML=`
  ...
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none" 
   ...
   <input style="display:none;" type="text" id="date" name="date" required>
      ...
      `,this.loadCalendar(),this.addEventListeners()}show(){var e=document.getElementById("scheduleevent");e&&(e.className="calendar-form-submit loading")}hide(){var e=document.getElementById("scheduleevent");e&&(e.className="calendar-form-submit")}async callApi({method:e="GET",url:t,date:a}){try{return await(await fetch(this.apiUrl+"/"+t,{method:e,headers:{accept:"*/*","Content-Type":"application/json","site-key":this.config.sitekey},...a?{body:JSON.stringify(a)}:{}})).json()}catch(e){return console.log("error",e),null}}addEventListeners(){var e=document.querySelector("#form"),e=(e&&e.addEventListener("submit",async e=>{e.preventDefault();e=new FormData(e.target.closest("form")),e=Object.fromEntries(e);try{this.show();var t=(e.name||"").trim().split(" "),a=t[0]||"",s=1<t.length?t.slice(1).join(" "):"",n=await this.callApi({method:"POST",url:"appointments",date:{...e,firstName:a,lastName:s,duration:60}});if(this.hide(),n&&n?._id)return this.renderSuccess();alert("Error scheduling event. Please try again later.")}catch(e){this.hide(),alert("An error occurred while scheduling the event. Please try again later.")}}),document.querySelectorAll(".calendar-navigation span")),e=(e&&0<e.length&&e.forEach(e=>{e.addEventListener("click",()=>{this.month="calendar-prev"===e.id?this.month-1:this.month+1,this.date=new Date(this.year,this.month,(new Date).getDate()),this.year=this.date.getFullYear(),this.month=this.date.getMonth(),this.loadCalendar()})}),document.getElementById("backBtn"));e&&e.addEventListener("click",()=>{this.render()})}async loadCalendar(){var e,t={};for(e of(await this.callApi({method:"GET",url:"holidays?limit=365&page=1",date:null}))?.data){var[a,s,n]=parseDate(e?.date);a===this.year&&s===this.month&&(t[n]=e)}var i,l=document.querySelector(".calendar-dates"),r=document.querySelector(".calendar-current-date"),o=new Date(this.year,this.month,1).getDay(),c=new Date(this.year,this.month+1,0).getDate(),d=new Date(this.year,this.month,c).getDay(),h=new Date(this.year,this.month,0).getDate(),u=new Date,m=(console.log("nextWeek",u),u.setDate(u.getDate()+7),console.log("nextWeek",u),u.getMonth()),p=u.getDate(),y=u.getFullYear();let v="";for(let e=o;0<e;e--)v+=`<li class="inactive">${h-e+1}</li>`;for(let e=1;e<=c;e++)e in t?v+=`<li alt="${t[e].description}" class="inactive">${e}</li>`:y===this.year&&(this.month<m||this.month===m&&e<=p)?v+=`<li  class="inactive">${e}</li>`:(i=new Date(this.year,this.month,e).setHours(0,0,0,0)>(new Date).setHours(0,0,0,0)&&0!==new Date(this.year,this.month,e).getDay()&&6!==new Date(this.year,this.month,e).getDay()?"active":"",v+=`<li 
      id="${this.year}-${this.month+1}-${e}" 
      class="${i}" onclick="selectDate(${this.year},${this.month+1},${e})">${e}</li>`);for(let e=d;e<6;e++)v+=`<li class="inactive">${e-d+1}</li>`;r.innerText=this.months[this.month]+" "+this.year,l.innerHTML=v}}async function selectDate(t,a,s){console.log("selectDate",{year:t,month:a,day:s});let e=new Date(t,a-1,s);console.log("selectDate",e);var n=new Date;if(console.log("today",n),n.setHours(0,0,0,0),e.setHours(0,0,0,0),e<n)alert("You cannot schedule a meeting in the past.");else if(0===e.getDay()||6===e.getDay())alert("You cannot schedule a meeting on Saturday or Sunday.");else{n=document.querySelectorAll(".active-event"),n=(n&&n.forEach(e=>e.classList.toggle("active-event")),t+`-${a}-`+s),n=(document.getElementById(n).classList.toggle("active-event"),document.querySelector(".calendar-events")),n=(n&&(n.style.display="flex"),document.querySelector(".calendar-events-date"));if(n){let e=new Date(t,a,s);n.innerText=e.toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}var i,n=await Api({url:`https://api-cso.zagrosagency.xyz/appointments/eventes/${t}-${a}-${s}T${(new Date).getHours()}:${(new Date).getMinutes()}:00.000Z`,method:"GET"});console.log("eventsTime",n),n.length<=0?alert("Error!You cannot schedule a meeting in the past."):(i=document.querySelector(".calendar-times"))&&(i.innerHTML=`
         ${n?.map(e=>`
